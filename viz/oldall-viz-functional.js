...
function allViz(period) {
  ...
  d3.json("php/student-worksheets.php", function(json) {
       render(json);
  });

// "php/student-worksheets.php",
  ...
  // Nest data by day and get average overall
     ...
     nestedOverallData.forEach(function (el) {
      ...
      yScaleOverall = d3.scale.linear().domain([yScaleOverallMax(), 0]).range([margin, height - margin]);
...
// ------------------------------------------
    ...
    svgTotalWorkG = d3.select(".total-work")
     ...
     d3.select(".total-work > g")
      ...
      var periodAverageScore = d3.mean(periodData, function(d) {
          return d.Percentage;}
        );
      ...
      d3.select(".average-score > g")
        ...
        .text(function(){
          if (averageScorePercentageChange > 0) {
              return  "" + averageScorePercentageChange + "% better than last " + periodReadable() + ""
          } else if (averageScorePercentageChange < 0) {
              return  "" + Math.abs(averageScorePercentageChange) + "% less than last " + periodReadable() + ""
          } else {
                return "No change from last "+ periodReadable() + "";
          }
          });
 ...
 d3.json("php/parent-logins.php", function(d2) {
  ...
  // ------------------------------------------
    ...
    // Create legend/key for overview visualisation - these are at the end to make sure that everything has been draw before these are applied
      ...
      if (sLoginHealth > 4 && pLoginHealth > 0 && averageHealth > 69 && wsHealth > 39) {
        overallHealth = '3';
      }
      else if (sLoginHealth > 4 && pLoginHealth > 2 && averageHealth > 50 && wsHealth > 19) {
        overallHealth = '2'
      }
      else {
        overallHealth = '1'
